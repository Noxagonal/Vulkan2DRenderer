...
)
{
	...
	// Collect old surface and swapchain, we'll destroy them later
	...
	// (Re-)Create synchronization semaphores
	...
	// (Re-)Create synchronization fences
	if( data->gpu_to_cpu_frame_fences.size() != data->swapchain_image_count ) {
	...
	WindowCreateInfo	&	window_create_info )
{
	...
	// Create surface
	{
		...
		vkGetPhysicalDeviceSurfaceSupportKHR(
			...
			data->renderer->data->primary_render_queue.queueFamilyIndex,
			data->surface,
			&surface_supported
		);
	...
	// Create render pipelines
	{
		...
		shader_stages[ 0 ].module					= data->renderer->data->vertex_shader_module;
		...
		shader_stages[ 1 ].module					= data->renderer->data->fragment_shader_module;
		...
		pipeline_create_info.layout					= data->renderer->data->pipeline_layout;
	...
	// Create command pool
	{
		...
		command_pool_create_info.queueFamilyIndex	= data->renderer->data->primary_render_queue.queueFamilyIndex;
	...
	if( !CreateSwapchain(
		...
		if( data->report_function ) {
			data->report_function( ReportSeverity::NON_CRITICAL_ERROR, "Cannot create swapchain!" );
		}
		return;
	}
...
VK2D_API bool VK2D_APIENTRY Window::BeginRender()
{
	...
	if( data->next_image == data->previous_image &&
		data->previous_frame_need_synchronization ) {
		if( vkWaitForFences(
			data->renderer->data->device,
